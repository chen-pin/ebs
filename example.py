...
def nemati2020_vvc6():
    ...
    hip_numbers = [
        config["targets"][star]["HIP"] for star in config["targets"]
    ]
    luminosities = [
        config["targets"][star]["luminosity"] for star in config["targets"]
    ]
    ...
    for i, star in enumerate(config["targets"]):
        spectral_dict[config["targets"][star]["HIP"]] = config["targets"][
            star
        ]["spec_type"]
    ...
    angles = np.linspace(
        config["angles"]["start"], config["angles"]["stop"], num_angles
    )  # Angular separation [as]
    wfe = np.sqrt(1.72**2 / num_temporal_modes) * np.ones(
        (num_temporal_modes, num_spatial_modes)
    )  # [pm]
    ...
    contrast_filename = os.path.join(
        config["paths"]["input"], error_budget.contrast_filename
    )
    throughput_filename = os.path.join(
        config["paths"]["input"], config["input_files"]["throughput"]
    )
    ...
    prompt = (
        "Loop over contrast [c], throughput [t], IWA [i], or dark noise [d]?  "
    )
    sel = input(prompt)
    ...
    if sel == "c":
        sweep = ParameterSweep(
            ...
            wfsc_factor=wfsc_factor,
            fixed_contrast=None,
            fixed_throughput=core_throughputs[1],
            contrast_filename=contrast_filename,
            throughput_filename=throughput_filename,
            angles=angles,
        ...
        sweep = ParameterSweep(
            ...
            values=core_throughputs,
            error_budget=error_budget,
            wfe=wfe,
            sensitivity=sensitivity,
            wfsc_factor=wfsc_factor,
            fixed_contrast=contrasts[1],
            fixed_throughput=None,
            contrast_filename=contrast_filename,
            throughput_filename=throughput_filename,
            angles=angles,
            output_file_name="example_core_tputs",
        )
        result_dict = sweep.run_sweep()
        print(result_dict["int_time"])
