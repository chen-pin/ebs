...
def plot_ebs_output(
    ...
    if plot_by_spectype and num_types > 1:
        for i, type in enumerate(unique_types_to_plot):
            use_stars = [
                ...
                if spectral_dict[key].startswith(unique_types_to_plot[i])
            ]
            plot_panel(
                axes[i],
                error_budget,
    ...
    plt.suptitle(
        f"Required Integration Time (hr, SNR={error_budget.exosims_pars_dict['observingModes'][0]['SNR']}) vs. "
...
def plot_panel(
    ...
    fill=False,
    plot_text=None,
    colors=np.array(["#01a075", "#00cc9e", "#6403fa", "#ff9400", "#cf5f00"]),
):
    """Plots the EBS parameter that was swept over as a function of calculated integration time.
    ...
    if (
        np.abs(np.nanmax(values) / np.nanmin(values)) > 100
        and not force_linear
    ):
        semilog_x = True

    if (
        np.abs(np.nanmax(int_times_for_calc) / np.nanmin(int_times_for_calc))
        > 100
        and not force_linear
    ):
        semilog_y = True
    ...
    if semilog_y or loglog:
        y_lim = 24 * np.nanmin(int_times_for_calc) * 0.5, np.min(
            [24 * max * 10, 1000]
        )
    ...
    for i, (k, v) in enumerate(spectral_dict.items()):
        if i in use_idxs:
            txt = "HIP%s\n%s, EEID=%imas" % (
                k,
                v,
                np.round(error_budget.eeid[i] * 1000),
            )

            ax.scatter(
                values,
                24 * int_times_for_calc[:, i, 0],
                c=colors[i],
                marker="o",
                s=70,
            )
    ...
    for i, (k, v) in enumerate(spectral_dict.items()):
        if i in use_idxs:
            legend_elements.append(
                (
                    Line2D(
                        [0], [0], color=colors[i], lw=2, label=f"HIP {k}, {v}"
                    ),
                )
            )
        else:
