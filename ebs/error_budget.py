...
for arguments `contrast_filename`, and
...
class ExosimsWrapper:
    ...
    def run_exosims(self, temp_json_filename, target_list, eeid, eepsr, exo_zodi, npoints, working_angles,
        ...
        sInds = np.array([np.where(sim.TargetList.Name == t)[0][0] for t in target_list])
    ...
    def write_temp_input_json(self, exosims_dict, filename='temp.json'):
        """
...
class ErrorBudgetMcmc(ExosimsWrapper):
    ...
    def __init__(self, config_file="config.yml", file_cleanup=True):
        super().__init__()
